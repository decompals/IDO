...
/* A hack for mips2 libc - which is only used in SPEC benchmarks.
 * setjmp and longjmp use ld/sd to load/store the fp context. Hence
 * the jmp_buf must be double-word aligned.
 */
typedef __uint64_t jmp_buf[_JBLEN/2];
#endif
