/*****************************************************************************
 *
 * File:	createmovieArgs.c++
 *
 * Description: Part of createmovie. Command line processing functions,
...
static compScheme   compressionScheme = mvc1;
static MVloopmode   loopMode 	      = MV_LOOP_NONE;
static MVfileformat movieFormat       = MV_FORMAT_SGI_3;
...
extern void processCmdArgs( int argc, char **argv )
{
    ...
    int ch;
    while ( ( ch = getopt( argc, argv, "c:l:r:s:p:o:" ) ) != -1 ) {
        switch ( ch ) {
            case 'c':
...
static void usage( void )
{
    ...
    fprintf( stderr, "[-s xsize,ysize] [-p paramType,userParam,userParamVal]" );
    fprintf( stderr, " -o outMovie file . . .\n" ); 
    fprintf( stderr, "compression = none, mvc1, mvc2, rle, jpeg, or 8rgb. " );
    fprintf( stderr, " Default is mvc1.\n" );
    fprintf( stderr, "loopMode = once, loop, or swing. Default is once.\n");
