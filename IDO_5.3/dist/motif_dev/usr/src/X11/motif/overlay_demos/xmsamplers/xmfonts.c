...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xmfonts.c,v $ $Revision: 2.4 $ $Date: 1993/02/26 22:13:27 $"
#endif
#endif
/*
...
#ifdef OVERLAY_DEMO
        windows[window_count++] = XtWindow(app_shell);
        XSetWMColormapWindows(XtDisplay(app_shell), XtWindow(app_shell),
...
Widget		parent;		/*  parent widget	*/
{
	...
	*/
	n = 0;
#ifdef OVERLAY_DEMO
        SG_getPopupArgs (XtDisplay(menu_bar), NULL, args, &n);
        menu_pane = XmCreatePulldownMenu (menu_bar, "menu_pane", args, n);
        windows[window_count++] = XtWindow(menu_pane);
#else /* OSF original */
	menu_pane = XmCreatePulldownMenu (menu_bar, "menu_pane", args, n);
#endif /* OVERLAY_DEMO */
...
#ifdef OVERLAY_DEMO
        SG_getPopupArgs(XtDisplay(XtParent(XtParent(parent))), NULL, args,
&n);
#endif /* OVERLAY_DEMO */
