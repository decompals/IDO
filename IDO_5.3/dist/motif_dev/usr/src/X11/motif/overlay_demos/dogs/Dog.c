...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Dog.c,v $ $Revision: 1.4 $ $Date: 1993/02/26 22:34:03 $"
#endif
#endif
...
#ifdef OVERLAY_DEMO /*sgi */
/* We are using different visuals. The depth won't be the default depth */
    ...
    XtIntervalId *t;
{
    ...
    _DogDrawPixmap(w);
    XtAppAddTimeOut (XtWidgetToApplicationContext((Widget)w),
#ifdef __sgi /* Added typing to keep the compiler happy */
		     WagTime(w), (XtTimerCallbackProc)do_wag, w);
#else /* OSF original */
		     WagTime(w), do_wag, w);
#endif /* sgi */
}
