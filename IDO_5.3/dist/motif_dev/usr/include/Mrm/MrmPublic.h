 ...
 * Motif Release 1.2.4
*/ 
/*   $RCSfile: MrmPublic.h,v $ $Revision: 1.5 $ $Date: 1994/05/18 01:22:42 $ */
...
/*
 * The following types are ones OSF is using now.
 * These prevent some core dumps due to unions and long/int truncation.
 *
 * OSF uses #defines for them -- seems like trouble to me, so we use typedef's.
 */
typedef INT16		MrmCode;	/* Used for codes, e.g. Mrmcr... */
typedef CARD8		MrmSCode;	/* Short code for small ranges */
typedef CARD16		MrmOffset;	/* Used for offsets in records */
typedef INT16		MrmType;	/* Used for types, e.g. MrmrType... */
typedef CARD16		MrmSize;	/* For size fields */
#ifndef __sgi /* OSF original */
typedef INT32		MrmCount;	/* For counter fields */
#else
typedef INT16		MrmCount;	/* For counter fields */
#endif /* __sGI */
typedef BOOL		MrmFlag;	/* flag fields */
typedef INT32		MrmResource_id;	/* Resource id in IDB files */
typedef INT16		MrmGroup;	/* For Mrm resource groups */
#endif /* __sgi */
