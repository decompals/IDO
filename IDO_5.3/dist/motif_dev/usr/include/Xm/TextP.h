 ...
 * Motif Release 1.2.4
*/ 
/*   $RCSfile: TextP.h,v $ $Revision: 0.6 $ $Date: 1994/06/19 22:23:26 $ */
/*
...
#ifdef __sgi
/* We need limits.h to get a portable max long integer value */
#include <limits.h>
#endif /* __sgi */
...
#ifdef __sgi
/*
 * MAXINT is no longer in 1.2.4.
 * It should be removed from here once the .c files are brought up to 1.2.4
 * LONG_MAX is from limits.h.
 */
#define MAXINT	LONG_MAX /* Biggest number that can fit in long */
...
typedef struct __SG_XmTextExtPart
  {
    Pixel   selection_background; /* color of the selected text background */
    Pixel    selection_foreground; /* color of the selected text foreground */
    Pixel    error_background;/* color of the selected error text background */
    Boolean  cursor_visible_on_focus; /* If true then cursor is visible
				       only when is has the keyboard focus */
    Boolean  post_motion_call;  /* If true then extra textVerifyCallback is made */
  } _SG_XmTextExtPart;

typedef struct __SG_XmTextExt
  {
    _SgInstanceExtensionRec	common;   /* Stuff all instance rec's have */
    _SG_XmTextExtPart	rsrc;	  /* Resources & instance var's */
  } _SG_XmTextExtRec, *_SG_XmTextExt;

#define _SG_TextPtr(w) \
((_SG_XmTextExt)(((XmTextWidget)(w))->text._SG_vendorExtension))
...
#ifdef __sgi
/* Allow future extensions without breaking shared library compatibility */
    _SG_XmTextExt _SG_vendorExtension;
#endif /* __sgi */
} XmTextPart;
