/*
 *	fromgif -
 *		convert a GIF file into an IRIS image file.
 *
 ...
 *
 *		cc -I/usr/include/gl fromgif.c -o fromgif -limage -lm
 *
 *	Use fromgif to convert from name.gif to name.rgb file.
 *	Then use "ipaste" to display the image.
 *
 */
#include "image.h"
#include "math.h"
...
char *argv[];
{
    ...
    makegamtab(GIFGAMMA);
    if(argc<2) {
	fprintf(stderr,"usage: fromgif image.gif image.rgb\n");
	exit(1);
    }
    filename = argv[1];
    imagename = argv[2];
...
rastertoimg(raster,image,xsize,ysize)
...
#define DRAWSEGMENT(offset, step)                       \
    ...
    image = iopen(imagename,"w",RLE(1),3,width,height,3);
    if(!image) {
	fprintf(stderr,"fromgif: can't open output image %s\n",imagename);
	exit(1);
    }
    rastertoimg(newras,image,width,height);
    iclose(image);
