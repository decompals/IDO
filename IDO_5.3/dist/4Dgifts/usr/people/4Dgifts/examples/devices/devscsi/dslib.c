...
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_REWI, 0, 0, 0, 0, 0);
  filldsreq(dsp, 0, 0, DSRQ_READ|DSRQ_SENSE);
  return(doscsireq(getfd(dsp), dsp));
}
...
int
requestsense03(struct dsreq *dsp, caddr_t data, long datalen, int vu)
{
...
int
read08(struct dsreq *dsp, caddr_t data, long datalen, long lba, int vu)
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_READ, 0, B2(lba), B1(datalen), B1(vu<<6));
...
int
write0a(struct dsreq *dsp, caddr_t data, long datalen, long lba, int vu)
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_WRIT, 0, B2(lba), B1(datalen), B1(vu<<6));
...
int
inquiry12(struct dsreq *dsp, caddr_t data, long datalen, int vu)
{
...
int
modeselect15(struct dsreq *dsp, caddr_t data, long datalen, int save, int vu)
{
...
int
reservunit16(struct dsreq *dsp, caddr_t data, long datalen, int tpr,
	     int tpdid, int extent, int res_id, int vu)
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_RESU, ((tpr&1)<<4) | ((tpdid&7)<<1) | (extent&1), res_id, B2(datalen), B1(vu<<6));
  filldsreq(dsp, data, datalen, DSRQ_WRITE|DSRQ_SENSE);
  return(doscsireq(getfd(dsp), dsp));
}
...
releaseunit17(struct dsreq *dsp, int tpr, int tpdid, int extent,
	      int res_id, int vu)
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_RELU, ((tpr&1)<<4) | ((tpdid&7)<<1) | (extent&1), res_id, 0, 0, B1(vu<<6));
  filldsreq(dsp, 0, 0, DSRQ_READ|DSRQ_SENSE);
  return(doscsireq(getfd(dsp), dsp));
}
...
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_MSEN, 0,
...
{
  fillg0cmd(dsp, CMDBUF(dsp), G0_SD,
...
int
readcapacity25(struct dsreq *dsp, caddr_t data, long datalen, long lba,
	       int pmi, int vu)
{
...
void
bprint(unsigned char *s, int n, int nperline, int space)
{
	int   i, x;
	char  *sp = (space) ? " ": "";
