...
int			 duration= (bell->duration*32)/1000;
	...
	cmn_err(CE_WARN,"Couldn't allocate streams buffer. Spaceball bell not rung.\n");
    }
    return 1;
}
...
mblk_t			*mbp;
	...
	if (send_reset)	{
		strcpy(mbp->b_wptr,SB_RESET_CMD);
	...
	cmn_err(CE_WARN,"Couldn't allocate streams buffer.  Spaceball not Reset.\n");
    }
    return 1;
}
...
short	nBytes,nWords,lastByte;
	...
	if (!packed) {
		...
		cmn_err(CE_WARN,"unexpected byte 0x%x in packed mode\n",str[i]);
		return(0);
	    }
	    spill<<= 6;
	    spill|= (val6&0x3f);
...
short	nBytes,nWords,lastByte;
	...
	if (str[i]=='^') {
	    if (i==len-1) {
		cmn_err(CE_WARN,"Error! binary mode string ends in '^'\n");
		return(0);
	    }
	    i++;
	    switch (str[i]) {
		...
		case '^': byteVal= '^'; break;
		default:  cmn_err(CE_WARN,"illegal binary sequence \"^%c\"\n",
								str[i]);
			  return(0);
	    }
	}
...
int	n;
    ...
    if (!sb->parseData) {
	cmn_err(CE_WARN,"data event with unknown format.\n");
	sb_issue_reset(sb,1);
	sb->parseData= sb_ParsePrintableWords;
    }
    n= (*sb->parseData)(len,str,&period,&vals.value[0]);
    if ((n!=3)&&(n!=6)) {
	cmn_err(CE_WARN,"Bad length for data event (%d words)\n",n);
	sb_issue_reset(sb,0);
	return;
    }
...
#define SEND_SBSTRING(s,len)  \
	...
	cmn_err(CE_WARN,\
	"Couldn't allocate streams buffer -- spaceball not initialized.\n");\
    }
...
register int	 len= sb->msgLen;
	...
	case 'C': 
		if (len>1) {
			...
			cmn_err(CE_WARN,"Unknown communications mode %c\n",
								str[1]);
			sb_issue_reset(sb,1);
		    }
		}
	...
	case 'K':
		if (len==3) {
		    ...
		    cmn_err(CE_WARN,"Illegal key event ignored.\n");
		    sb_issue_reset(sb,1);
		}
		break;
	case 'D':
	...
	case 'Z':
		...
		break;
	case 'E':
		cmn_err(CE_WARN,"Error \"%s\" occured\n",&str[1]);
		break;
	...
	case '@':
		...
		break;
	case '?':
		cmn_err(CE_WARN,"Error message \"%s\" ignored\n",&str[1]);
		break;
#ifdef NOTDEF
	case 'B': case 'b':
	...
	case 'Q': case 'q':
		...
		break;
	default:
		cmn_err(CE_WARN,"Unknown packet \"%c...\" ignored\n",str[0]);
		break;
#endif /* NOTDEF */
    }
    return;
...
mblk_t			*mbp;
	...
	if (mbp= allocb(size,BPRI_MED)) {
	    strcpy(mbp->b_wptr,pCtrl->data);
	    mbp->b_wptr+= strlen(mbp->b_wptr);
	    putnext( sb->info.sInfo.wq, mbp );
	}
	else {
	    cmn_err(CE_WARN,"Couldn't allocate streams buffer -- spaceball control ignored.\n");
	}
    }
...
sb_open( queue_t *rq, dev_t *dev, int flag, int sflag, struct cred *cred )
{
	...
	if ( !rq->q_ptr ) {
		...
		sb->info.sInfo.readData = sb_intr;
