...
void
fly(perfobj_t *viewer_pos)
{
    float       terrain_z, xpos, ypos, xcntr, ycntr;
    float	delta_speed = .0006;

    if(SkyType == SKY_CYCLOPS)
	delta_speed *= .5;
	
    xcntr = Wxorg + Wxsize/2;
    ycntr = Wyorg + Wysize/2;
    ...
    if (Xgetbutton(PERIODKEY)) {
	while (Xgetbutton(PERIODKEY))
	    ;
	printf("X=%f Y=%f Z=%f Azimuth=%f Speed=%f\n", X, Y, Z, Azimuth, Speed);
    }
	...
	if(SkyType == SKY_DUALCHANNEL) {
    	    xpos = (Xgetvaluator(MOUSEX)-(1279-(float)Wxsize/2)) / 
							((float)Wxsize*14.);
	    ypos = (Xgetvaluator(MOUSEY)-512) / (Wysize*.5);
	} else {
...
void
fly_paper_planes(perfobj_t *paper_plane_pos)
{
    int             	i;
    float		speed = .08;
    float		terrain_z;
...
static float 
terrain_height(float x, float y)
{
