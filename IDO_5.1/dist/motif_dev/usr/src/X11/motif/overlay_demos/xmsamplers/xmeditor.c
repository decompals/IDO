...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xmeditor.c,v $ $Revision: 2.6 $ $Date: 1993/02/26 22:13:22 $"
#endif
#endif
/*
...
#ifdef OVERLAY_DEMO
/*
 * We need the depth so we can get the right pixmap from XmGetPixmapByDepth().
 * The right thing to do would probably be to use XtGetValues on the parent.
 * For now, we cheat, and include CoreP.h, to define the widget structure.
 */
...
#ifdef OVERLAY_DEMO
  /*
   * This is to keep information for an eventual call to XSetWMColormapWindows.
   * We need that call in order that:
   *    * pull-downs get the right colormap at all
   *    * transients come up the right color
   */
...
#ifdef OVERLAY_DEMO
  pixmap = XmGetPixmapByDepth(XtScreen(warning_dialog), image_string,
                       foreground, background, warning_dialog->core.depth);
#else
...
#ifdef OVERLAY_DEMO
/*
 * Note that this will set all menus from this menubar into the overlay
 * planes.  The reason is that all pulldowns from a single menubar share
 * the same shell.  Thus they are necessarily all in the same visual.
 */
  SG_getPopupArgs (XtDisplay(parent), NULL, args, &n);
  menu_pane1 = XmCreatePulldownMenu(menu_bar, "menu_pane1", args, n);
/* Save a window (actually a colormap) to be noted to XSetWMColormapWindows() */  windows[window_count++] = XtWindow(menu_pane1);
#else /* OSF original */
  menu_pane1 = XmCreatePulldownMenu(menu_bar, "menu_pane1", args, n);
#endif /* OVERLAY_DEMO */
...
#ifdef OVERLAY_DEMO
/* Tell the window manager about the non-default colormaps we need. */
  windows[window_count++] = XtWindow(Shell1);
  XSetWMColormapWindows(XtDisplay(Shell1), XtWindow(Shell1),
