...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xmprotocol.c,v $ $Revision: 2.5 $ $Date: 1993/02/26 22:13:48 $"
#endif
#endif
/*
...
#ifdef __sgi /* allow for being called by pathname */
    char *progname;
...
#ifdef __sgi /* use pathname component */
    display = XtOpenDisplay(app_context, NULL,
                progname, "XmDemos", NULL, 0, &argc, argv);
#else /* OSF Original */
    display = XtOpenDisplay(app_context, NULL,
...
#ifdef OVERLAY_DEMO
/*
 * No very interesting transient stuff,
 * so put the entire demo in the overlay planes.
 */
    i = 0;
    SG_getPopupArgs (display, NULL, myArgs, &i);
    toplevel = XtAppCreateShell(progname, "XMdemos",
		applicationShellWidgetClass,
                display, myArgs, i);
#else
    toplevel = XtAppCreateShell(progname, NULL, applicationShellWidgetClass,
                display, NULL, 0);
#endif /* OVERLAY_DEMO */
#else /* OSF Original */
...
#ifdef OVERLAY_DEMO
/*
 * Note that border color defaults to black (in the normal planes).
 * Since this is pixel 0, in the overlays it is transparent.
 * either set the border width to zero or else set the color
 * explicitly.  We set the color in the app defaults file.
 */
#endif /* OVERLAY_DEMO */
