...
#ifdef _NO_PROTO
	static VisInfoPtr SG_getVisualList();
	static Visual *SG_getVisual();
	static int SG_getMatchingClass();
	static int SG_getMatchingDepth();
	static int SG_getMatchingType();
	static void SG_warning();
#else/* _NO_PROTO */
	static VisInfoPtr SG_getVisualList
		(Display *dpy, int scr);
	static Visual *SG_getVisual
	...
	int       *n;
{
	...
	if ((int)visual<0) return ((int)visual);
	...
	int       *n;
{
	...
	if ((int)visual<0) return (SG_getPopupArgs(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);
	...
	int       *n;
{
	...
	depth = 4;
	visual = SG_getVisual(dpy,scr, depth, &PColor, SG_OVERLAY_PLANES);
	if ((int)visual<0) return (SG_getOverlay2Args(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);
	...
	int       *n;
{
	...
	if ((int)visual<0) return (SG_getNormalArgs(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);
...
{
	static XVisualInfo   vTemplate;		/* Visual values we need */
	VisInfoPtr vp;
	register int i;
