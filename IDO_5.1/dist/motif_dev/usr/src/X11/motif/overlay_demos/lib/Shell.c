...
#ifdef sgiShlib
/* Since Shell.c is a separable widget, rename the globals to an alternate
 * name, and put indirect calls to them in ShellData.c
 * it is defined in import.def.h, so first undefine it and then redefine it
 * to use an internal name.  Within this file there is no need to go through
 * the import mechanism, so the define affects all internal calls.
 */
#undef _XtShellGetCoordinates
#define _XtShellGetCoordinates __sgiShlib_XtShellGetCoordinates
#endif
...
static ShellClassRec __sgi_shellClassRec = {
#else
externaldef(shellclassrec) ShellClassRec shellClassRec = {
#endif
  {   /* Core */
    /* superclass	  */	sgi_SUPERCLASS((WidgetClass) &compositeClassRec),
    /* class_name	  */	"Shell",
    ...
    /* set_values_almost  */	sgi_SHARED_INITIALIZER_XtInheritSetValuesAlmost,  
    ...
    /* insert_child	  */	sgi_SHARED_INITIALIZER_XtInheritInsertChild,
    /* delete_child	  */	sgi_SHARED_INITIALIZER_XtInheritDeleteChild,
    /* extension	  */	NULL
  },{ /* Shell */
...
static OverrideShellClassRec __sgi_overrideShellClassRec = {
#else
externaldef(overrideshellclassrec) OverrideShellClassRec overrideShellClassRec = {
#endif
  {
    /* superclass         */    sgi_SUPERCLASS((WidgetClass) &shellClassRec),
    ...
    /* initialize         */    NULL,
    /* initialize_notify    */	NULL,		
    /* realize            */    sgi_SHARED_INITIALIZER_XtInheritRealize,
    /* actions            */    NULL,
    /* num_actions        */    0,
    ...
    /* visible_interest   */    FALSE,
    /* destroy            */    NULL,
    /* resize             */    sgi_SHARED_INITIALIZER_XtInheritResize,
    /* expose             */    NULL,
    /* set_values         */    NULL,
    /* set_values_hook      */	NULL,			
    /* set_values_almost    */	sgi_SHARED_INITIALIZER_XtInheritSetValuesAlmost,  
    ...
    /* geometry_manager   */    sgi_SHARED_INITIALIZER_XtInheritGeometryManager,
    /* change_managed     */    sgi_SHARED_INITIALIZER_XtInheritChangeManaged,
    /* insert_child	  */	sgi_SHARED_INITIALIZER_XtInheritInsertChild,
    /* delete_child	  */	sgi_SHARED_INITIALIZER_XtInheritDeleteChild,
    /* extension	    */  NULL
  },{
...
static WMShellClassRec __sgi_wmShellClassRec = {
#else
...
static TransientShellClassRec __sgi_transientShellClassRec = {
#else
externaldef(transientshellclassrec) TransientShellClassRec transientShellClassRec = {
#endif
  {
    /* superclass	  */	sgi_SUPERCLASS((WidgetClass) &vendorShellClassRec),
    ...
    /* intrinsics version */	XtVersion,
    /* callback offsets	  */	NULL,
    /* tm_table		  */	sgi_SHARED_INITIALIZER_XtInheritTranslations,
    /* query_geometry	  */	NULL,
    /* display_accelerator*/	NULL,
    /* extension	  */	NULL
  },{
...
static TopLevelShellClassRec __sgi_topLevelShellClassRec = {
#else
...
static ApplicationShellClassRec __sgi_applicationShellClassRec = {
#else
externaldef(applicationshellclassrec) ApplicationShellClassRec applicationShellClassRec = {
#endif
  {
    /* superclass         */    sgi_SUPERCLASS((WidgetClass) &topLevelShellClassRec),
...
#ifdef sgiShlib
/*
 * This initialization function is needed to handle values not fully
 * resolved when the shared library is created.  It is listed in, and
 * called from __SgiSharedXtInit(), which is automatically updated to call
 * any routine of the form __SgiSharedXtInit_whatever.  When modifying this
 * routine, be sure that the class record assignment preceeds any
 * individual field declarations.  For more information see
 * ../../util/shlib/README
 */
__SgiSharedXtInit_Shell() {
    /* because the widget can be replaced, check the first field of the
     * shell class rec.  If it is non zero, the user has linked in one,
     * so we can return.  If it is 0, we must initialize it.
     */
    if (shellClassRec.core_class.superclass != NULL)
	return;

    /* shell */
    shellClassRec = __sgi_shellClassRec;
    sgi_SUPERCLASS_INIT(shellClassRec,&compositeClassRec);
    sgi_SHARED_INIT(shellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(shellClassRec.composite_class.insert_child,
		    XtInheritInsertChild);
    sgi_SHARED_INIT(shellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);

    /* overrideShell */
    overrideShellClassRec = __sgi_overrideShellClassRec;
    sgi_SUPERCLASS_INIT(overrideShellClassRec,&shellClassRec);
    sgi_SHARED_INIT(overrideShellClassRec.core_class.realize,XtInheritRealize);
    sgi_SHARED_INIT(overrideShellClassRec.core_class.resize,XtInheritResize);
    sgi_SHARED_INIT(overrideShellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(overrideShellClassRec.composite_class.geometry_manager,
		    XtInheritGeometryManager);
    sgi_SHARED_INIT(overrideShellClassRec.composite_class.change_managed,
		    XtInheritChangeManaged);
    sgi_SHARED_INIT(overrideShellClassRec.composite_class.insert_child,
		    XtInheritInsertChild);
    sgi_SHARED_INIT(overrideShellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);

    /* wmShell */
    wmShellClassRec = __sgi_wmShellClassRec;
    sgi_SUPERCLASS_INIT(wmShellClassRec,&shellClassRec);
    sgi_SHARED_INIT(wmShellClassRec.core_class.realize,XtInheritRealize);
    sgi_SHARED_INIT(wmShellClassRec.core_class.resize,XtInheritResize);
    sgi_SHARED_INIT(wmShellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(wmShellClassRec.composite_class.geometry_manager,
		    XtInheritGeometryManager);
    sgi_SHARED_INIT(wmShellClassRec.composite_class.change_managed,
		    XtInheritChangeManaged);
    sgi_SHARED_INIT(wmShellClassRec.composite_class.insert_child,
		    XtInheritInsertChild);
    sgi_SHARED_INIT(wmShellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);

    /* transientShell */
    transientShellClassRec = __sgi_transientShellClassRec;
    sgi_SUPERCLASS_INIT(transientShellClassRec,&vendorShellClassRec);
    sgi_SHARED_INIT(transientShellClassRec.core_class.resize,XtInheritResize);
    sgi_SHARED_INIT(transientShellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(transientShellClassRec.core_class.tm_table,
		    XtInheritTranslations);
    sgi_SHARED_INIT(transientShellClassRec.composite_class.geometry_manager,
		    XtInheritGeometryManager);
    sgi_SHARED_INIT(transientShellClassRec.composite_class.change_managed,
		    XtInheritChangeManaged);
    sgi_SHARED_INIT(transientShellClassRec.composite_class.insert_child,
		    XtInheritInsertChild);
    sgi_SHARED_INIT(transientShellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);

    /* topLevelShell */
    topLevelShellClassRec = __sgi_topLevelShellClassRec;
    sgi_SUPERCLASS_INIT(topLevelShellClassRec,&vendorShellClassRec);
    sgi_SHARED_INIT(topLevelShellClassRec.core_class.realize,XtInheritRealize);
    sgi_SHARED_INIT(topLevelShellClassRec.core_class.resize,XtInheritResize);
    sgi_SHARED_INIT(topLevelShellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(topLevelShellClassRec.core_class.tm_table,
		    XtInheritTranslations);
    sgi_SHARED_INIT(topLevelShellClassRec.composite_class.geometry_manager,
		    XtInheritGeometryManager);
    sgi_SHARED_INIT(topLevelShellClassRec.composite_class.change_managed,
		    XtInheritChangeManaged);
    sgi_SHARED_INIT(topLevelShellClassRec.composite_class.insert_child,
		    XtInheritInsertChild);
    sgi_SHARED_INIT(topLevelShellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);

    /* applicationShell */
    applicationShellClassRec = __sgi_applicationShellClassRec;
    sgi_SUPERCLASS_INIT(applicationShellClassRec,&topLevelShellClassRec);
    sgi_SHARED_INIT(applicationShellClassRec.core_class.realize,XtInheritRealize);
    sgi_SHARED_INIT(applicationShellClassRec.core_class.resize,XtInheritResize);
    sgi_SHARED_INIT(applicationShellClassRec.core_class.set_values_almost,
		    XtInheritSetValuesAlmost);
    sgi_SHARED_INIT(applicationShellClassRec.core_class.tm_table,
		    XtInheritTranslations);
    sgi_SHARED_INIT(applicationShellClassRec.composite_class.geometry_manager,
		    XtInheritGeometryManager);
    sgi_SHARED_INIT(applicationShellClassRec.composite_class.change_managed,
		    XtInheritChangeManaged);
    sgi_SHARED_INIT(applicationShellClassRec.composite_class.delete_child,
		    XtInheritDeleteChild);
}
#endif /* sgiShlib */
...
#ifdef __sgi
/*
 * In the standard libXt.a, the Shell widget inherits:
 *	* from its widget parent: depth, colormap, etc
 *	* from the >>ROOT WINDOW<< : visual
 *
 * The net effect is that if you are using different visuals, your visual
 * may be inconsistent with other things (e.g. depth).  When this happens,
 * the X server returns a BadMatch error fron the XCreateWindow call.
 *
 * The scenario where this occurs is in such things as cascading menus, or
 * menus that then pop up dialog boxes.
 *
 * This violates the mental model that as long as one is working in widget-land,
 * all copying/inheritance of resource values should be via the widget tree.
 *
 * The following simple change fixes this problem:
 *
 *	To make sure things match, we look back up the widget tree.  If we
 *	get here and "visual" is not yet set, we look for the most recent
 *	ancestor (if any) which is a subclass of Shell, and has it set.
 *
 * This also has the advantage that the application programmer can set a widget
 * into a different visual, and expect anything down the tree from there to
 * default into the same visual.
 */
	if ( !w->shell.visual ) {
		ShellWidget ww = (ShellWidget)w->core.parent;
		while ( ww  && (!XtIsShell(ww) || !ww->shell.visual) )
				ww = (ShellWidget)ww->core.parent;
		if (ww) w->shell.visual = ww->shell.visual;
	}
#endif /* __sgi */
