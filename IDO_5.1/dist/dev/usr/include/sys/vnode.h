...
typedef struct vnode {
	...
	short		v_listid;		/* index into vfreelist */
	void		*v_data;		/* private data for fs */
	struct filock	*v_filocks;		/* ptr to filock list */
	sema_t		*v_filocksem;		/* ptr to semaphore for list */
	struct pregion	*v_mreg;		/* mapped file region pointer */
	u_long		v_version;		/* vnode number capability */
	u_long		v_number;		/* in-core vnode number */
	struct vnode	*v_hash;		/* hash list for lookup */
	lock_t		v_lock;			/* spinlock for v_count */
	sema_t		v_sync;			/* synchronization semaphore */
} vnode_t;
...
#define VWASMAP		0x100	/* vnode was mmapped and may still be */
#define VMOLDY		0x200	/* dir with a moldy MAC label (mac_enabled) */
...
typedef struct vnodeops {
	...
	void	(*vop_rwlock)(vnode_t *);
	void	(*vop_rwunlock)(vnode_t *);
	...
	int	(*vop_map)(vnode_t *, u_int, struct pregion *, char **,
			   u_int, u_int, u_int, u_int, struct cred *);
	int	(*vop_addmap)(vnode_t *, u_int, struct pregion *, addr_t,
			      u_int, u_int, u_int, u_int, struct cred *);
	int	(*vop_delmap)(vnode_t *, u_int, struct pregion *, addr_t,
			      u_int, u_int, u_int, u_int, struct cred *);
	...
	int	(*vop_allocstore)(struct vnode *, u_int, u_int, struct cred *);
	/* sgi extension */
...
#define IO_DIRECT	0x04	/* bypass page cache */
#define IO_IGNCACHE	0x08	/* ignore page cache coherency when doing i/o
					(IO_DIRECT) */
...
typedef struct vattr {
	...
	struct mac_label *va_label;	/* MAC label for B1 and beyond */
} vattr_t;
...
#define AT_LABEL	0x8000
#define AT_UPDATIME	0x010000
#define AT_UPDMTIME	0x020000
#define AT_UPDCTIME	0x040000

#define	AT_ALL	(AT_TYPE|AT_MODE|AT_UID|AT_GID|AT_FSID|AT_NODEID|\
		AT_NLINK|AT_SIZE|AT_ATIME|AT_MTIME|AT_CTIME|\
		AT_RDEV|AT_BLKSIZE|AT_NBLOCKS|AT_VCODE|AT_LABEL)
...
extern int	vn_rdwr(enum uio_rw, struct vnode *, caddr_t, int, off_t,
			enum uio_seg, int, long, struct cred *, int *);
...
extern u_long		vn_epoch;		/* incore vnode capability -- */
...
typedef struct vnode_map {
	u_long		v_number;		/* in-core vnode number */
	u_long		v_version;		/* vnode number capability */
	u_long		v_epoch;		/* epoch in vnode history */
} vmap_t;
...
extern vnode_t	**vhash;	/* hash buckets for active vnodes	*/
extern long	vhashmask;	/* active vnode hash mask		*/
 ...
 */
#define	VN_LOCK(vp)		splock((vp)->v_lock)
#define	VN_UNLOCK(vp, s)	spunlock((vp)->v_lock, s)
...
extern int cstat(struct vnode *, struct irix4_stat *, struct cred *);
extern int xcstat(struct vnode *, struct irix5_stat *, struct cred *);
