...
extern void fawltysysend(short, struct sysent *, sysarg_t *, long, int);
...
extern void fawltyvmfault(pid_t,char,caddr_t,caddr_t);
extern int vmfaultflags;
#define VMFAULTTRACE(sc,va,epc) {if ((trflags & VMFAULTHIST) && \
				     (vmfaultflags & VMEVENT_FLAG(sc))) \
					fawltyvmfault(u.u_procp->p_pid,\
						      sc,va,(caddr_t) epc);}
#define VMSWAPTRACE(pid,sc,va,epc) {if ((trflags & VMFAULTHIST) && \
				     (vmfaultflags & VMEVENT_FLAG(sc))) \
					fawltyvmfault(pid, \
						      sc,va,(caddr_t) epc);}
#else
#define VMFAULTTRACE(sc,va,epc) { 0; }
#define VMSWAPTRACE(pid,sc,va,epc) { 0; }
#endif
 ...
 * want infinite amounts of data - we limit it to PARY_MAXINDBYTES
 */
#define PAR_MAXINDBYTES	64
struct callrec {
	short	pid;
	unchar	numparams;
	unchar	token;
