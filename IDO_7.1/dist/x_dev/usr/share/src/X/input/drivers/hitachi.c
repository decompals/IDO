 ...
 */
#define HTCHI_SETUP       "\001SU"
#define HTCHI_CUSTOM	  "\001CU"
#define HTCHI_HIII	  "\001HJ"
#define HTCHI_MODE	  "\001DF\001FU\001AD"	/* upper-left origin, disable buzz */
#define HTCHI_BINARY	  "\001BI\001D8"	/* 7-byte binary mode */
#define HTCHI_RESOLUTION  "\001MT\001HE\001L2"  /* mm, high res, 0.01mm */
#define HTCHI_PRESSURE    "\001PP\001ZE\001TE"  /* Pressure, height & tilt */
#define HTCHI_RATE        "\001R1"		/* max output rate */
#define HTCHI_STREAM      "\001CN"		/* run mode */
#define HTCHI_INCR        "\001IC"		/* incremental mode */
#define HTCHI_EXECUTE	  "\001EX"
/*
 * We may have to add a RESET command at the end.  If so, we must
 * add a half-second delay after sending EX, before we send RS
 */
/* #define HTCHI_RESET	  "\001RS" */
...
mblk_t                  *mbp;
    ...
    if (!idevChangeLineSettings(&htchi->info,&_htchi_termio)) {
        cmn_err(CE_WARN,"Hitachi tablet: initialization failed.\n");
        return 0;
    }
    if (mbp = allocb(sizeof(HTCHI_SETUP), BPRI_MED)) {
	strcpy((char *) mbp->b_wptr, HTCHI_SETUP);
	putnext(htchi->info.sInfo.wq, mbp);
	STREAMS_DELAY(HZ/2);
	if (mbp = allocb(sizeof(HTCHI_CUSTOM), BPRI_MED)) {
	    strcpy((char *) mbp->b_wptr, HTCHI_CUSTOM);
	    putnext(htchi->info.sInfo.wq, mbp);
	    STREAMS_DELAY(HZ/2);
	    if (mbp = allocb(sizeof(HTCHI_HIII), BPRI_MED)) {
		strcpy((char *) mbp->b_wptr, HTCHI_HIII);
		putnext(htchi->info.sInfo.wq, mbp);
		STREAMS_DELAY(HZ/2);
		if (mbp= allocb(HTCHI_INIT_LEN,BPRI_MED)) {
		    strcpy((char *) mbp->b_wptr,HTCHI_MODE);
		    strcat((char *) mbp->b_wptr,HTCHI_BINARY);
		    strcat((char *) mbp->b_wptr,HTCHI_RESOLUTION);
		    strcat((char *) mbp->b_wptr,HTCHI_PRESSURE);
		    strcat((char *) mbp->b_wptr,HTCHI_RATE);
		    strcat((char *) mbp->b_wptr,HTCHI_STREAM);
		    strcat((char *) mbp->b_wptr,HTCHI_INCR);
		    strcat((char *) mbp->b_wptr,HTCHI_EXECUTE);
    ...
    cmn_err(CE_WARN,"Hitachi tablet: couldn't allocate a streams buffer. Initialization failed\n");
    return 0;
}
...
/* ARGSUSED */
static void
htchi_pressure_intr( idevInfo *pInfo, char *strin, int len )
{
    ...
    while (len--) {
		 ...
		 */
                ...
                break;
            default:
                cmn_err(CE_WARN,"Hitachi tablet: invalid state %d!!\n",htchi->state);
                htchi->state=HITACHI_NOT_SYNCHED;
                break;
        }
    }
...
/* ARGSUSED */
static void
htchi_intr( idevInfo *pInfo, char *strin, int len )
{
    htchi_state_t *htchi = (htchi_state_t *)pInfo;
    unsigned char *str = (unsigned char *) strin;
