...
#define EXTERN_SYM
#else
#define EXTERN_SYM extern
#endif

/*
 * some stuff for NDS
 */
EXTERN_SYM int kbd_index;
EXTERN_SYM int kbd_minor;
EXTERN_SYM int bogus_kbd_minor;
EXTERN_SYM int bogus_kbd_index;
...
kbd_other_query(kbd_state_t *kbd, idevOtherControl *pCtrl)
{
	...
	if ((strcmp(pCtrl->name,"keymap")==0)&&(kbd->map[0]!='\0')) {
	    strncpy(pCtrl->data,(char *)kbd->map,IDEV_CTRL_DATA_LEN);
	    pCtrl->data[IDEV_CTRL_DATA_LEN]= '\0';
	    return 1;
	}
	return 0;
	...
	switch ( cmd ) {
		...
		case IDEVGETKEYMAPDESC:
			...
			if (size>=sizeof(idevKeymapDesc)) {
				...
				strncpy(desc->name,(char *)kbd->map,
 ...
 */
kbd_idev_wput( register queue_t *wq, mblk_t *mp)
{
