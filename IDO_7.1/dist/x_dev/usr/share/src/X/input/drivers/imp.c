...
/* ARGSUSED */
static void
imp_intr( idevInfo *pInfo, char *str, int len )
{
...
#ifdef DEBUG
				cmn_err( CE_WARN, "imp: received bad data" ) ;
#endif /* DEBUG */
				imp->state = IMP_BUTTONS;
				imp_osync++;
				continue;
...
struct iocblk	*iocb;
	...
	if (iob==NULL) {
		cmn_err(CE_WARN,"imp: can't allocate streams buffer for M_IOCTL. Line settings not changed.\n");
		return 0;
	}
	iob->b_cont= allocb( sizeof(struct termio), BPRI_MED );
	if (iob->b_cont==NULL) {
		cmn_err(CE_WARN,"imp: can't allocate streams buffer for M_DATA. Line settings not changed.\n");
		freemsg(iob);
		return 0;
	}
