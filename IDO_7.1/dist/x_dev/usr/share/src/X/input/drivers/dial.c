...
dial_enable_buttons(dial_state_t *dial, register idevBitVals *btns)
{
	...
	if (mbp = allocb(5,BPRI_MED)) {
		...
		cmn_err(CE_WARN,"dial: alloc failed, active buttons not changed\n");
		return 0;
	}
	if (mbp= allocb(5,BPRI_MED)) {
		dcmd = mbp->b_wptr;
...
dial_enable_valuators(dial_state_t *dial, register idevBitVals *vals)
{
	...
	if (mbp= allocb(3,BPRI_MED)) {
		...
		cmn_err(CE_WARN,"dial: alloc failed, active dials not changed\n");
		return 0;
	}
	return 1;
}
...
dial_set_resolution(dial_state_t *dial, register idevGetSetValDesc *vals)
{
	...
	if (mbp!=NULL) {
		...
		cmn_err(CE_DEBUG,"dial: Couldn't allocate streams buffer.  Dial filter value not changed.\n");
	}
    }
    return 1;
}
...
dial_set_LEDs(dial_state_t *dial, register idevBitVals *LEDs)
{
	...
	if (mbp = allocb(5,BPRI_MED)) {
		...
		cmn_err(CE_WARN,"dial: Couldn't allocate streams buffer -- LEDs not changed.\n");
		return 0;
	}
	return 1;
}
...
dial_set_str_dpy(dial_state_t *dial, register idevStrDpyState *dpy)
{
	...
	if (dpy->nDpy == 0) {
		...
		if (mbp= allocb(DIAL_STR_DPY_MAX_LEN+1,BPRI_MED)) {
			...
			cmn_err(CE_WARN,"dial: Couldn't allocate streams buffer -- string display not changed\n");
			return 0;
		}
		return 1;
	}
...
dial_init_device(dial_state_t *dial, int size, char *stuff)
{
	...
	if (!idevChangeLineSettings(&dial->info,NULL)) {
		cmn_err(CE_WARN,"dial: Dial box not initialized.\n");
		return 0;
	}
	if (mbp= allocb(1,BPRI_MED)) {
		mbp->b_wptr[0] = DIAL_INITIALIZE;
		mbp->b_wptr += 1;
		putnext( dial->info.sInfo.wq, mbp );
	}
	else {
		cmn_err(CE_WARN,"dial: Couldn't allocate streams buffer -- dial box not initialized\n");
		return 0;
	}
	dial->initialized= 1;
...
dial_check_init( dial_state_t *dial )
{
	if ((!dial->ready)&&(dial->checkinit_id)) {
		...
		cmn_err(CE_WARN,"dial: Dial box not responding.\n");
	    }
	}
	dial->checkinit_id= 0;
        return;
	...
	while (len > 0) {
		...
		if ((dial->state != 0) || IS_DIAL_EVENT(ch)) {
			switch (dial->state) {
				...
				default:
					cmn_err(CE_WARN,"dial: Impossible state %d in dial_intr.\n");
					dial->state = 0;
					break;
			}
			dial->errorCount = 0;
		...
		else if (IS_INIT_EVENT(ch)) {
			dial_setup(dial);
			dial->errorCount = 0;
		}
		else {
			cmn_err(CE_WARN,"dial: unexpected char %d from dial\n",ch);
			if (dial->errorCount++>5) { /* try to reinitialize */
				cmn_err(CE_WARN,"dial: Reinitializing dial box\n");
				dial_init_device(dial,0,NULL);
			}
		}
		len--;
...
dial_open( queue_t *rq, dev_t *dev, int flag, int sflag, struct cred *cred )
{
	...
	if ( !rq->q_ptr ) {
		...
		dial->info.sInfo.readData = (void (*)())dial_intr;
