...
<!ENTITY	chp.text-b4	CDATA	"if(isempty(attr(LBL,ancestor(CHAPTER))),'',switch(attr(LANG,ancestor(SGIDOC)),de,Kapitel ,fr,Chapitre ,ja_JP.EUC,‘æ ,DEFAULT,Chapter ) attr(LBL,ancestor(CHAPTER))) if(isempty(attr(LBL,ancestor(CHAPTER))),'',switch(attr(LANG,ancestor(SGIDOC)),ja_JP.EUC, Í,DEFAULT,))"	>
...
<!ENTITY	index.text-b4	CDATA	"switch(attr(LANG,ancestor(SGIDOC)),de,Index,fr,Index,ja_JP.EUC,õˆø,DEFAULT,Index)"	>
