...
)
{
	...
	case DW_FORM_ref4 : 
	    READ_UNALIGNED(offset, attr->ar_debug_info_ptr, sizeof(Dwarf_ufixed));
	    break;
...
)
{
	...
	case DW_FORM_ref_udata :
	    ...
	    */
            READ_UNALIGNED(ref_addr, attr->ar_debug_info_ptr, dbg->de_length_size);
            offset =  ref_addr;
	    break;
	default :
	    _dwarf_error(dbg,error,DW_DLE_BAD_REF_FORM);
...
)
{
	...
	case DW_FORM_block4 :
	    READ_UNALIGNED(length, attr->ar_debug_info_ptr, sizeof(Dwarf_ufixed));
...
)
{
	...
	if(0 == dbg->de_assume_string_in_bounds) {
	  /* Check that string lies within current cu in .debug_info. */
          void *end = dbg->de_debug_info + cu_context->cc_debug_info_offset +
            cu_context->cc_length + dbg->de_length_size;
