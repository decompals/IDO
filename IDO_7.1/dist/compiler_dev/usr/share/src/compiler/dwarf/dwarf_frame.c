/*
    ...
    $Revision: 1.52 $ $Date: 1996/07/23 23:26:39 $
*/
	...
	fp_offset = 0;
        switch (opcode) {
			...
			*returned_error = (DW_DLE_DF_NO_CIE_AUGMENTATION);
			...
			READ_UNALIGNED(adv_loc, instr_ptr, sizeof(Dwarf_ufixed));
...
)
{
    ...
    READ_UNALIGNED(cie_id, cie_ptr, dbg->de_length_size);
    SIGN_EXTEND(cie_id, dbg->de_length_size);
    if (cie_id != DW_CIE_ID) {
...
)
{
    ...
    Dwarf_Unsigned  length = 0;		/* READ_UNALIGNED needs 8 byte dest */
    ...
    */
	...
	SIGN_EXTEND(offset, dbg->de_length_size);
	    ...
	    */
		...
		DECODE_LEB128_UWORD(frame_ptr, length_of_augmented_fields);
	   ...
	   */
	    ...
	    Dwarf_Signed offset_into_exception_tables = DW_DLX_NO_EH_OFFSET;
	    ...
	    /* Get the augmentation string from Cie to identify the layout
	       of this Fde. */
            cieptr = (Dwarf_Small *) (dbg->de_debug_frame + offset);
...
/* ?? THis presumes that the offset is always 32 bits */
		    READ_UNALIGNED(offset_into_exception_tables, frame_ptr, 
						sizeof(Dwarf_sfixed));
    ...
    *fde_element_count = fde_count;
	...
	Dwarf_Sword new_cie_index = cie_count;
...
)
{
    ...
    res = get_augmentation_string(dbg, 
		      (Dwarf_Small *) (dbg->de_debug_frame+offset),
		      &augmentation, error);
    if (res != DW_DLV_OK) {
	return res;
    }
	...
	DECODE_LEB128_UWORD(fde_ptr, length_of_augmented_fields);
	saved_fde_ptr = fde_ptr;
	if (strcmp((char *) augmentation, DW_CIE_AUGMENTER_STRING_V0) == 0) {
	    /* The first word is an offset into execption tables. */
	    READ_UNALIGNED(offset_into_exception_tables, fde_ptr, 
					sizeof(Dwarf_sfixed));
	    SIGN_EXTEND(offset_into_exception_tables,
					sizeof(Dwarf_sfixed));
	...
	if ((strcmp((char *) augmentation, 
	    ...
	    cie_ptr++;
	    /* Decode the length of augmented fields. */
	    DECODE_LEB128_UWORD(cie_ptr, length_of_augmented_fields);
	    ...
	    /* We do not understand the augmentation string. No
	       assumption can be made about any fields other than
	       what we have already read. */
	    cie_ptr = start_cie_ptr + length + dbg->de_length_size;
	    /* FIX -- What are the values of data_alignment_factor,
	      code_alignement_factor, return_address_register and
	      instruction start? They were clearly uninitalized in 
	      the previous version and I am leaving them the same way. */
...
/* Return the register rules for all registers at a given pc. */
{
	...
	if (cie->ci_initial_table == NULL) {
        ...
        res = _dwarf_exec_frame_instr(/*make_instr=*/false, NULL, 
		/*search_pc*/false, 
		/*search_pc_val*/0, 
		/*location*/0,
            cie->ci_cie_instr_start, 
            cie->ci_cie_start + cie->ci_length + dbg->de_length_size,
            cie->ci_initial_table, cie, dbg,&icount,&dw_err);
	if(res == DW_DLV_ERROR) {
             ...
             Dwarf_Unsigned *outaddrlen, Dwarf_Error *error)
{
    ...
    len = (inFde->fd_fde_start + inFde->fd_length + dbg->de_length_size)
		- instrs;
...
)
{
	...
	returned_op_list,
        /*search_pc*/false, /*search_pc_val*/NULL, 
		/*location*/0,
