...
The purpose of this document is to propose a library of functions 
...
issues will be addressed seperately.
...
Dwarf_Handler:4|8:4|8:Pointer to libdwarf error handler 
...
In any implemenation this field *must* be as large as the
...
to the SGI-specfic \f(CWdwarf_get_vars()\fP
function, and are used as descriptors for 
...
that an error occured in executing the function, since
...
to a \f(CWlibdwarf\fP function (except where explicitely permitted), 
...
the \f(CWDwarf_Debug\fP descriptor sepcifying the object for which the
...
.\"size attribute that deliniates a range. For example: a debugging information 
...
a query indicates an unsucessful return, to determine the nature of the 
...
particular compilation-unit.  The compilation-unit is specified
...
of the DIE representing the pubname, and the compilation-unit header 
of the compilation-unit containing the pubname, respectively.  
On a 
...
It also returns in the locations 
pointed to by \f(CWdie_offset\fP, and \f(CWcu_offset\fP, the offsets
of the DIE representing the weak name, and the compilation-unit header 
of the compilation-unit containing the weak name, respectively.  
It returns \f(CWDW_DLV_ERROR\fP on error.  
It never returns \f(CWDW_DLV_NO_ENTRY\fP.
On a 
...
It also returns in the locations
pointed to by \f(CWdie_offset\fP, and \f(CWcu_offset\fP, the offsets
of the DIE representing the static function, and the compilation-unit 
header of the compilation-unit containing the static function,
respectively.  
It returns \f(CWDW_DLV_ERROR\fP on error.  
It never returns \f(CWDW_DLV_NO_ENTRY\fP.
...
It also returns in the locations
pointed to by \f(CWdie_offset\fP, and \f(CWcu_offset\fP, the offsets
of the DIE representing the user-defined type, and the compilation-unit 
header of the compilation-unit containing the user-defined type,
respectively.  
It returns \f(CWDW_DLV_ERROR\fP on error.  
It never returns \f(CWDW_DLV_NO_ENTRY\fP.
...
It also returns in the locations
pointed to by \f(CWdie_offset\fP, and \f(CWcu_offset\fP, the offsets
of the DIE representing the file-scope static variable, and the 
compilation-unit header of the compilation-unit containing the 
...
.TE
.FG "List of Dwarf 2 Error Codes (contd.)"
.DE
