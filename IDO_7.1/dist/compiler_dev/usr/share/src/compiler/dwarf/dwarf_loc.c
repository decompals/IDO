...
)
{
	...
	*/
	...
	switch (atom) {
	    ...
	    case DW_OP_addr :
				READ_UNALIGNED(operand1, loc_ptr,
				    dbg->de_length_size);
				loc_ptr += dbg->de_length_size;
				offset += dbg->de_length_size;
				break;
	    ...
	    case DW_OP_const2u :
				READ_UNALIGNED(operand1, loc_ptr, 2);
				loc_ptr = loc_ptr + 2;
				offset = offset + 2;
				break;
	    ...
	    case DW_OP_const4u :
				READ_UNALIGNED(operand1, loc_ptr, 4);
				loc_ptr = loc_ptr + 4;
				offset = offset + 4;
				break;
	    ...
	    case DW_OP_const8u :
				READ_UNALIGNED(operand1, loc_ptr, 8);
				loc_ptr = loc_ptr + 8;
				offset = offset + 8;
				break;
