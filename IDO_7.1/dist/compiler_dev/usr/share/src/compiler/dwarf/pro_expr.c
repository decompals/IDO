...
)
{
	...
	*/
	...
	case DW_OP_const2s :
	    operand = (Dwarf_Small *)&operand_buffer[0];
	    *(Dwarf_Half *)operand = 
		*(Dwarf_Half *)((char *)&val1 + sizeof(val1) - 2);
	    operand_size = 2;
	    break;
	...
	case DW_OP_const4s :
	    operand = (Dwarf_Small *)&operand_buffer[0];
	    *(Dwarf_ufixed *)operand = 
		*(Dwarf_ufixed *)((char *)&val1 + sizeof(val1) - 4);
	    operand_size = 4;
	    break;
...
)
{
    ...
    next_byte_offset = expr->ex_next_byte_offset + SIZEOF_UWORD(dbg) + 1;
    if (next_byte_offset > MAXIMUM_LOC_EXPR_LENGTH) {
    ...
    *next_byte_ptr = DW_OP_addr;
    next_byte_ptr++;
    memcpy(next_byte_ptr, ((char *)&addr + sizeof(addr) - SIZEOF_UWORD(dbg)),
	SIZEOF_UWORD(dbg));
