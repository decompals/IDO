/*
	pro_die.c
	$Revision: 1.15 $    $Date: 1996/07/23 23:26:43 $    
	$Source: /hosts/bonnie/proj/irix6.4-ssg/isms/cmplrs/libdwarf/RCS/pro_die.c,v $

	Contains die related routines 
	...
	Dwarf_Error *error)
{
	...
	if (IS_64BIT(dbg)) {
	    new_attr->ar_attribute_form = DW_FORM_data8;
	    new_attr->ar_rel_type = R_MIPS_64;
	}
	else {
	    new_attr->ar_attribute_form = DW_FORM_data4;
	    new_attr->ar_rel_type = R_MIPS_32;
	}
	    
	new_attr->ar_nbytes = sizeof_uword(dbg);
	new_attr->ar_next = NULL;
	new_attr->ar_data = (char *)
	    _dwarf_p_get_alloc(NULL, sizeof_uword(dbg));
	if (new_attr->ar_data == NULL) {
	    DWARF_P_DBG_ERROR(NULL,DW_DLE_ADDR_ALLOC,DW_DLV_NOCOUNT);
	}
	if (IS_64BIT(dbg)) {
	    Dwarf_Unsigned du = 0;
	    memcpy((void *)new_attr->ar_data,(const void *)&du,sizeof(Dwarf_Unsigned));
	}
	else {
	    Dwarf_ufixed dw = 0;
	    memcpy((void *)new_attr->ar_data,(const void *)&dw,sizeof(Dwarf_ufixed));
	}
