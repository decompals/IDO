...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xmter.c,v $ $Revision: 1.4 $ $Date: 1995/02/10 02:15:37 $"
#endif
#endif
/***************************************************************************
*  Xmter.
...
#ifdef __sgi
/*
 * This sets up the demo with no window manager borders.  The demo also has
 * no other way to get a menu and exit.  (MB3 gets a speed-control slider.)
 * Oh, well -- it's a demo.
 *
 * N.b. The slider popup itself is created with window namager decorations.
 */
#endif /* __sgi */
char *fallback[] = {
  "*mwmDecorations:  0",
...
XtPointer callback_data ;
{
  ...
  if ((dacb->event->type == ButtonPress) &&
	...
	speed_dialog = XmCreateFormDialog(toplevel, 
					  "Speed control",
					  NULL, 0) ;
#else /* Set up resource so the popup has a title bar, so it can be moved. */
	n = 0;
	XtSetArg(args[n], XmNmwmDecorations, MWM_HINTS_STATUS);  n++ ;
	speed_dialog = XmCreateFormDialog(toplevel, "Speed control", args, n) ;
#endif /* __sgi */
	n = 0 ;
