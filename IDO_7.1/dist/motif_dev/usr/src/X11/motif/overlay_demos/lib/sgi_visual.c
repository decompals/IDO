 ...
 * define SG_IS_STATIC.
 *
 * This is the same as using "SG_STORAGE", which is defined in sgi_visual.h
 *
...
#ifdef _NO_PROTO
	SG_STORAGE VisInfoPtr SG_getVisualList();
	SG_STORAGE Visual *SG_getVisual();
	SG_STORAGE int SG_getMatchingClass();
	SG_STORAGE int SG_getMatchingDepth();
	SG_STORAGE int SG_getMatchingType();
	SG_STORAGE void SG_warning();
#else/* _NO_PROTO */
	SG_STORAGE VisInfoPtr SG_getVisualList
		(Display *dpy, int scr);
	SG_STORAGE Visual *SG_getVisual
...
SG_STORAGE Colormap
SG_getDefaultColormap (dpy, scr, vsl)
	Display *dpy;
	int      scr;
	Visual  *vsl;
{
...
SG_STORAGE int
SG_getDefaultDepth (dpy, scr, class, type)
	Display  *dpy;
	int       scr, *class, type;
{
	VisInfoPtr vp;
	...
	return SG_getMaxDepth(dpy, scr, class, type);
...
SG_STORAGE int
SG_getMatchingClass (dpy, scr, type, depth, class)
	Display  *dpy;
...
SG_STORAGE int
SG_getMatchingDepth (dpy, scr, class, type, depth)
	Display  *dpy;
	int       *class, scr, type, depth;
{
	...
	return SG_getDefaultDepth (dpy, scr, class, type);
...
SG_STORAGE int
SG_getMatchingType (dpy, scr, class, type, typeP)
	Display  *dpy;
	int	  *class, scr, type;
	char	**typeP;
{
...
SG_STORAGE XVisualInfo *
SG_getMatchingVisual (dpy, scr, vsl, class, type, depth)
	Display  *dpy;
...
SG_STORAGE int
SG_getMaxDepth (dpy, scr, class, type)
	Display  *dpy;
	int       scr, *class, type;
{
	VisInfoPtr vp;
...
SG_STORAGE int 
SG_getNormalArgs(dpy, scr, args, n)
	Display   *dpy;
	int       scr;
	ArgList   args;
	int       *n;
...
} /* END OF FUNCTION SG_getNormalArgs */
 ...
 */
SG_STORAGE int 
SG_getOverlayArgs(dpy, scr, args, n)
	Display   *dpy;
	int       scr;
	ArgList   args;
	int       *n;
{
	...
	if ((ptrdiff_t)visual<0) return (SG_getPopupArgs(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);
...
SG_STORAGE int 
SG_getOverlay2Args(dpy, scr, args, n)
	Display   *dpy;
	int       scr;
	ArgList   args;
	int       *n;
...
SG_STORAGE int 
SG_getOverlay4Args(dpy, scr, args, n)
	Display   *dpy;
	int       scr;
	ArgList   args;
	int       *n;
{
	...
	depth = 4;
	visual = SG_getVisual(dpy,scr, depth, &PColor, SG_OVERLAY_PLANES);
	if ((ptrdiff_t)visual<0) return (SG_getOverlay2Args(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);
...
SG_STORAGE int 
SG_getPopupArgs(dpy, scr, args, n)
	ArgList   args;
	int       scr;
	Display   *dpy;
	int       *n;
{
	...
	if ((ptrdiff_t)visual<0) return (SG_getNormalArgs(dpy, scr, args, n));
	colormap = SG_getDefaultColormap (dpy, scr, visual);

	XtSetArg(args[*n], XtNcolormap, colormap); (*n)++;
	XtSetArg(args[*n], XtNdepth, 2); (*n)++;
	XtSetArg(args[*n], XtNvisual, visual); (*n)++;
 ...
 */
SG_STORAGE int 
SG_getUnderlayArgs(dpy, scr, args, n)
	Display   *dpy;
	int       scr;
	ArgList   args;
	int       *n;
...
SG_STORAGE Visual *
SG_getVisual (dpy, scr, depth, v_class, v_type)
    Display	*dpy;
...
SG_STORAGE int 
SG_getVisualArgs(dpy, scr, depth, class, type, args, n)
    Display           *dpy;
    ...
    int               *n;
{
	...
	visual   = SG_getVisual (dpy, scr, depth, class, type);
	colormap = SG_getDefaultColormap (dpy, scr, visual);
...
SG_STORAGE VisInfoPtr
SG_getVisualList(dpy, scr)
	Display *dpy;
	int scr;
{
...
SG_STORAGE void
#ifdef _NO_PROTO
SG_warning (dpy, format, p1, p2, p3, p4)
    Display *dpy;
    char *format;
    char *p1, *p2, *p3, *p4;
#else /* _NO_PROTO */
SG_warning (Display *dpy, char *format, char *p1, char *p2, char *p3, char *p4)
#endif /* _NO_PROTO */
{
    ...
    fprintf (stderr, _SGI_INTL_MSG(  _SGI_MMX_mwm_SGwarn ), name, class, pch);
    fflush (stderr);
...
"SG_visualType \"%s\" contains a non-alphabetic character \"%c\".",
				...
				*cp = '\0';
				SG_warning (dpy, "Using \"%s\" instead.",
						requestedType, "", "", "");
#else /* SGI change */
				SG_warning (dpy,
					_SGI_INTL_MSG( _SGI_MMX_mwm_visual1 ),
					requestedType, (char *)*cp, "","");
				*cp = '\0';
				SG_warning (dpy, _SGI_INTL_MSG( _SGI_MMX_mwm_visual2 ),
						requestedType, "", "", "");
#endif /*__sgi */
				break;
			}
...
#else /* SGI change */
					SG_warning (dpy,
					_SGI_INTL_MSG( _SGI_MMX_mwm_noreq_visual ),
						requestedType,
						(char *)*requestedDepth, "","");
					SG_warning (dpy,
	_SGI_INTL_MSG( _SGI_MMX_mwm_visual3 ), "", "", "", "");
					*requestedDepth = DefaultDepth
						(dpy, scr);
#endif /*__sgi */
					*requestedClass =
...
"Unable to get requested visualType (%s) with visualDepth (%d).",
			...
			SG_warning (dpy,
				...
				requestedType, (char *)*requestedDepth, "", "");
			SG_warning (dpy,
			_SGI_INTL_MSG( _SGI_MMX_mwm_visual4 ),
				actualType, (char *)depth, "", "");
#endif /*__sgi */
		}
	}
	...
	*requestedClass = SG_getMatchingClass
			(dpy, scr, typeV, depth,
			 requestedClassWasNull ? NULL : requestedClass);
	setNonNull(requestedVisual, SG_getVisual
