...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xmdialogs.c,v $ $Revision: 2.5 $ $Date: 1993/02/26 22:13:17 $"
#endif
#endif
/*
...
/*-------------------------------------------------------------
**      DialogDestroyed
**              If a dialog is destroyed, clear out its entry.
**              This avoids problems if the dialog is destroyed
**              through some external medium, such as the close
**              menu item in mwm.
*/
void DialogDestroyed (w, client_data, call_data)
Widget          w;              /*  widget id           */
...
#ifdef OVERLAY_DEMO
/*
 * This puts the dialogs in the overlay planes.
 * Note that the selected item may NOT be a dialog,
 * and therefore can't be put in the overlay planes (no shell widget).
 *
 * This above code line (GetDialogArguments (al, &ac)) should really follow
 * our addition.  The problem is it assumes starting from a zero offset,
 * and we want to make minimal changes to get into the overlay planes.
 */
                        if (selected_item_is_dialog)
                                SG_getOverlayArgs (display, NULL, al, &ac);
#endif /* OVERLAY_DEMO */
			dialog[index] = (*create_proc[index]) (shell_parent,
						item[index], al, ac);
#ifdef __sgi
                /* If widget is externally destroyed (e.g. through close menu
                 * in mwm) we need to know about it.
                 */
                        XtAddCallback(dialog[index], XmNdestroyCallback,
                                      (XtCallbackProc)DialogDestroyed, &dialog[index]);
#endif /* __sgi */
			break;
		case DIALOG_DESTROY:
			if (dialog[index])
 ...
 */
                                if (selected_item_is_dialog)
                                        SG_getOverlayArgs(display,NULL, al,
&ac);
#endif /* OVERLAY_DEMO */
				dialog[index] =
...
Widget		parent;
{
	...
	*/
	...
	ac = 0;
#ifdef OVERLAY_DEMO
/* Put the "resize policy" option menu in the overlay planes */
        SG_getOverlayArgs (display, NULL, al, &ac);
#endif /* OVERLAY_DEMO */
