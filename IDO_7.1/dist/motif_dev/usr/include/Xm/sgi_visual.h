...
#	define SG_OPAQUE_OV_PLANES	5
#	define SG_NORMAL_TRANSPARENT_PLANES	6
#	define SG_MAX_TYPES		(SG_NORMAL_TRANSPARENT_PLANES + 1)
...
SG_STORAGE int      SG_getDefaultDepth
	(Display *dpy, int scr, int *_class, int type);
SG_STORAGE XVisualInfo *SG_getMatchingVisual
	(Display *dpy, int scr, VisualID vsl, int *_class, int type, int depth);
SG_STORAGE int      SG_getMaxDepth
	(Display *dpy, int scr, int *_class, int type);
SG_STORAGE int      SG_getNormalArgs
...
SG_STORAGE int      SG_getUnderlayArgs
	(Display *dpy, int scr, ArgList args, int *n);
SG_STORAGE int      SG_getVisualArgs
	(Display *dpy, int scr, int dpth, int *_class, int type, ArgList args, int *n);
#endif /* _NO_PROTO */
#undef SG_STORAGE
#ifdef __cplusplus
}
