 ...
 * Motif Release 1.2.4
*/ 
/*   $RCSfile: Mrm.h,v $ $Revision: 1.9 $ $Date: 1995/04/26 22:04:18 $ */
...
#ifdef __sgi
#ifdef MRM_LIBRARY_BUILD /* SGI upgrade for 64-bits */
/*
 * MRM strings are never more than 32 bits, so this definition does not hurt.
 * It does prevent "implicit conversion from "unsigned long" to ..." warnings.
 *
 * Ditto for "sizeof".
 */
#  define strlen(s) ((unsigned int)strlen(s))
#  define sizeof(s) ((unsigned int)sizeof(s))
#endif /* MRM_LIBRARY_BUILD */
#endif /* __sgi */
...
/*
 * NOTE: WORD64 does not apply to SGI -- it is only for machines with a
 * 64-bit integer.  To date (4/95) SGI's ints are all 32-bit, even on
 * "64-bit" machines.
 */

/*
 * The following is what OSF is currently using.
 * This cleanup is because of errors, including core dumps, when doing
 * 64-bit stuff.
 *
 * OSF used #define's, rather than typedef's.  Seems like a bad idea, so
 * I am keeping typedef's for now.
 */

/*
 * Record number. 65k should be enough...
 */
typedef CARD16 IDBRecordNumber ;
 ...
 * in the given resource map.
 */
typedef CARD16 IDBResourceIndex ;

typedef union {
	CARD32			external_id ;	/* Looks like a fullword */
        struct {
          IDBRecordNumber       map_rec;	/* resource map record */
          IDBResourceIndex      res_index;	/* resource index into map */
        }                       internal_id ;	/* 2 fields internally */
 ...
 * A resource ID passed by value in a fullword.
 */
typedef CARD32 IDBResource ;	/* a declaration for RIDs passed by value */
...
#else /* SGI upgrade for 64-bits */
	/* Update to OSF's latest 5/6/94 */
	CARD32		variety ;/* Regular widget or auto child. */
	CARD32		annex ; 	/* annex allows compatible growth */
#endif /* __sgi */
...
typedef struct {
	...
	union {
#ifndef __sgi /* OSF original */
	  unsigned	ival ;		/* integer, boolean, compressed */
#else /* SGI upgrade for 64-bits */
	/* Update to OSF's latest 5/6/94 */
	  long		ival ;		/* integer, boolean, compressed */
#endif /* __sgi */
...
extern String Urm__CW_DisplayToString _ARGUMENTS(( char *val ,
						    String add_string ,
						    int add_string_size ));
extern Cardinal Urm__CW_ConvertValue  _ARGUMENTS(( long *val , 
						    MrmType reptype , 
						    MrmType cvttype , 
						    Display *display , 
						    MrmHierarchy hierarchy_id , 
