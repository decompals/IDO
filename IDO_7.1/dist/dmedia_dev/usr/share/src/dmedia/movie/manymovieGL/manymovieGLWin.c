 ...
 *		This program supports playing up to 4 movies simultaneously.
 *
...
/*
 * The X Display and OpenGL context. Local to this module.
 */
...
static DMstatus initMovie( char *name, int movieNum );
static void 	mapWindows( void );
static DMstatus createXWindow( int width, int height, int movieNum,
			       Window *winReturn );
...
static void initManyWindows( void )
{
    ...
    for( i = 0; i < numMovies; i++ ) { 
        ...
        if ( initMovie( name, i ) != DM_SUCCESS ) {
            exit( EXIT_FAILURE );
        }
...
static void initOneWindow( void )
{
    ...
    if ( createXWindow( winWidth, winHeight, 0, &theWin ) != DM_SUCCESS ) {
...
static DMstatus bindWinToMovies( Window win )
{
    ...
    for( i = 0; i < numMovies; i++ ) {
    	...
    	ctxt[ i ] = glXCreateContext( dpy, vis, 0, GL_TRUE );
    	if ( ctxt[ i ] == NULL ) {
        ...
        if ( mvBindOpenGLWindow( theMovie, dpy, win, ctxt[ i ] )
	    != DM_SUCCESS ) {
...
static DMstatus initMovie( char *name, int movieNum )
{
    ...
    winWidth =  movieWidth + 2 * WIN_MARGIN;
    winHeight = movieHeight + 2 * WIN_MARGIN;

    if ( createXWindow( winWidth, winHeight, movieNum, &tmpWin )
	!= DM_SUCCESS ) {
        fprintf( stderr, "%s: Could not create window.\n", getProgramName() );
	return DM_FAILURE;
    }
    ...
    ctxt[ movieNum ] = glXCreateContext( dpy, vis, 0, GL_TRUE );
    if ( ctxt == NULL ) {
        fprintf( stderr, "%s: Could not create rendering context.\n",
                getProgramName() );
        exit( EXIT_FAILURE );
    ...
    if ( mvBindOpenGLWindow( theMovie, dpy, tmpWin, ctxt[ movieNum ] ) 
	!= DM_SUCCESS ) {
	fprintf( stderr, "%s: Could not bind movie to window.\n",
		 getProgramName() );
	return DM_FAILURE;
    ...
    mvSetViewSize( theMovie, winWidth, winHeight, DM_TRUE );
			      ...
			      Window *winReturn )
{
    ...
    *winReturn = XCreateWindow( dpy, RootWindow( dpy, vis->screen ),
                         0, 0, width, height,0,vis->depth,
			 InputOutput, vis->visual, 
                         CWColormap|CWBorderPixel, &childWinAttribs );

    XSetWindowBackground( dpy, *winReturn, black );
...
/*********
 *
 * Retrieve the GLXContext.
 *
 *********/

GLXContext getGLXContext( int movieNum )
{
    return ctxt[ movieNum ];
}
