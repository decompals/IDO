...
)
{
  ...
  if ( QueryExtension("GLX_EXT_visual_info") ) {
      ...
      GL_GET_CONFIG(GLX_TRANSPARENT_INDEX_EXT, &glVisual->transparentIndex);
...
)
{
  ...
  if (result = GetGLVisualInfo(dpy, visual, gl)) {
	...
	printf("Accum=(%d,%d,%d,%d)",
       ...
       */
      if (!hasRGB)
	printf("ERROR: RGBA visual, but RGB have zero size\n");
      if (hasAlpha && hasAccum && gl->accumAlphaSize == 0)
	printf("ERROR: RGBA visual with Alpha and Accum, but Accum Alpha size is zero\n");
      if (!hasAlpha && hasAccum && gl->accumAlphaSize != 0)
	printf("ERROR: RGBA visual without Alpha and with Accum, but Accum Alpha size is not zero\n");
    }				       /* end rgba visual */
    /*
     * COLOR INDEX visual
     */
	...
	printf("Stencil=%d", gl->stencilSize);
       ...
       */
      if (hasRGB)
	printf("ERROR: CI visual, but RGB has non-zero sizes (%d,%d,%d)\n",
	       gl->redSize, gl->greenSize, gl->blueSize);
      if (hasAlpha)
	printf("ERROR: CI visual, but Alpha has non-zero size = %d\n",
	       gl->alphaSize);
      if (hasAccum)
	printf("ERROR: CI visual, but Accum has non-zero sizes (%d,%d,%d,%d)\n",
