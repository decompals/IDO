...
static void lists_alloc();
static void lists_fill();
...
static void scene_draw(int rend)
{
  ...
  if (draw_ring) {
    ...
    if (rend) {
      if (use_textures) glDisable(GL_TEXTURE_2D);
      glDisable(GL_COLOR_MATERIAL);
    }
  }
...
{
  RGBImageRec *teximage = NULL;
  
  teximage = rgbImageLoad((char *)texfile_stones);
  ...
  teximage = rgbImageLoad((char *)texfile_ground);
  ...
  teximage = rgbImageLoad((char *)texfile_trees);
  ...
  teximage = rgbImageLoad((char *)texfile_telescope);
