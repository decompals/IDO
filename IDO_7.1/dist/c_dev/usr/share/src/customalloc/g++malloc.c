...
extern void*     memcpy(void*,  const void*, int);
       void      bzero(void* s, int l) { memset(s, 0, l); }
#else
...
static size_t       request2size(size_t       request)
{
...
static mbinptr size2bin(size_t       sz)
{
  mbinptr b = av;
...
static void set_size(mchunkptr p, size_t       sz)
{
...
static void* chunk2mem(mchunkptr p) 
{ 
  set_inuse(p);
...
static void split(mchunkptr p, size_t       offset)
{
...
static void consollink(mchunkptr p)
{
...
static void frontlink(mchunkptr p)
{
  mbinptr   bn = size2bin(p->size);
  mchunkptr h  = &(bn->hd);
  mchunkptr f  = h->fd;
...
#ifdef _INTERNAL_MALLOC_
static void __internal_free(void* mem)
#else
...
#ifdef _INTERNAL_MALLOC_
static void* __internal_realloc(void* mem, unsigned int bytes)
#else
...
{
  if (malloc_pagesize == 0) malloc_pagesize = sysconf(_SC_PAGESIZE);
