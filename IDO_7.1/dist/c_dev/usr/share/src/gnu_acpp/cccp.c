...
int do_define (), do_line (), do_include (), do_undef (), do_error (),
  ...
  do_elif (), do_endif (), do_sccs (), do_ident(), do_once ();
		...
		*obp++ = '-';
		...
		while (1) {
		  ...
		  if (ibp == limit && ip->macro != 0) {
                    ...
                    RECACHE;
                    if(obp != lcl_obp) {
                        /* we reallocated: now must relocate
                                obufp_before_macroname to match the
			   reallocated obp and op->buf
                           Bug discovered  7/92. murthy test case.
                           fixed: davea
                        */
                        obufp_before_macroname -= (long)lcl_obp;
                        obufp_before_macroname += (long)obp;
                    }
		  }
     ...
     register U_CHAR *p;
{
  ...
  while (1) {
	...
	while (*p && *p++ != '\n') ;
		...
		/* Not white and not a comment: get a warning shortly
		   if warnings are on.  bug 97584 5/92.*/
		break;
      }
#endif
    } else break;
  ...
  while (1) {
	...
	*eofp = 1;
	...
	++*count_newlines;
      ...
      if (match == '\'') {
#ifndef __sgi
	/* While we are skipping, we will allow these unterminated strings
	   Behavior is in the -undefined- category so it is ok to
	   ignore these errors
	*/
	error_with_line (line_for_error (start_line),
     ...
     char *name;
{
  ...
  if (errno < sys_nerr) {
    if(errno == EMFILE && indepth > 10 ) {
      fprintf (stderr, "%s: "
	"Too many open files (%ld"
        " files is too many.  Does some #include include itself (directly or indirectly)?)\n",
	name, (long)indepth);
